...
export const Route = createFileRoute("/blog")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Free guides, ideas, and templates for final year projects — pricing, domain-wise ideas, report formats, and more.",
      },
...
function Blog() {
  return (
    <div className="mx-auto max-w-5xl px-4 py-14">
      ...
      <h1 className="mt-1 text-4xl font-black text-navy md:text-5xl">Final year project resources & ideas</h1>
      <p className="mt-3 max-w-2xl text-muted-foreground">
        Guides, templates and idea lists to help you pick, plan and ship your final year project.
      </p>
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {posts.map((p) => (
          <article key={p.title} className="rounded-2xl border border-border bg-card p-5 shadow-card transition-all hover:-translate-y-1 hover:border-gold">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Coming soon — subscribe below for launch.</p>
          </article>
        ))}
      </div>
      <div className="mt-12 rounded-3xl border border-border bg-card p-8 text-center shadow-card">
        ...
        <Link to="/contact" className="mt-4 inline-block rounded-full bg-navy px-6 py-3 text-sm font-semibold text-primary-foreground">
          Get in touch
        </Link>
      </div>
    </div>
